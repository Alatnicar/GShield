...
function Initialize-EventLog {
    if (-not [System.Diagnostics.EventLog]::SourceExists("GShield")) {
        ...
        Write-Log "Created Event Log source: GShield"
    }
}
...
function Apply-SecurityRules {
    ...
    try {
        ...
        $existingThreats = Get-MpThreatDetection | Where-Object { $_.ThreatName -like "GShield_*" }
    ...
    foreach ($indicator in $hashIndicators) {
        try {
            ...
            $tempFile = "$env:TEMP\GShield_threat_$hash.txt"
    ...
    foreach ($indicator in $fileIndicators) {
        try {
            ...
            $threatName = "GShield_File_$([System.IO.Path]::GetFileNameWithoutExtension($fileName))"
    ...
    Register-WmiEvent -Query "SELECT * FROM __InstanceCreationEvent WITHIN 1 WHERE TargetInstance ISA 'Win32_Process'" -Action {
        ...
        if ($fileNameHash.ContainsKey($processName)) {
            try {
                ...
                Write-EventLog -LogName "Application" -Source "GShield" -EventId 1001 -EntryType "Warning" -Message $logMessage
                ...
                Write-EventLog -LogName "Application" -Source "GShield" -EventId 1002 -EntryType "Error" -Message $errorMessage
            }
        }
    }
...
function Main {
    Initialize-EventLog
    Register-ScheduledTask -TaskName "RunGShieldAtLogon" -ScriptPath "C:\Windows\Setup\Scripts\Bin\GShield.ps1" -AtLogon
